...
public enum SignUpCommands {
    CREATE_ACCOUNT("\\s*create\\s+user(?=.*-u\\s+(?<username>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(?=.*-p (?<password>\"[^\\\"]*\"|[^\\s\\\"]*)\\s*(?<passwordConfirm>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(?=.*-e (?<email>\"[^\\\"]*\"|[^\\s\\\"]*))(?=.*-n (?<nickname>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(?=.*-s (?<slogan>\"[^\\\"]*\"|[^\\s\\\"]*))?(\\s+(-u (\"[^\\\"]*\"|[^\\s\\\"]*))|" +
            "(-p (\"[^\\\"]*\"|[^\\s\\\"]*)\\s+(\"[^\\\"]*\"|[^\\s\\\"]*))|(-e (\"[^\\\"]*\"|[^\\s\\\"]*))|" +
            "(-n (\"[^\\\"]*\"|[^\\s\\\"]*))|(-s (\"[^\\\"]*\"|[^\\s\\\"]*))?){4,5}\\s*"),
    PICK_SECURITY_QUESTION("\\s*question\\s+pick(?=.*-q\\s+(?<questionNumber>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(?=.*-a (?<answer>\"[^\\\"]*\"|[^\\s\\\"]*))(?=.*-c (?<answerConfirm>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(\\s+((-q (\"[^\\\"]*\"|[^\\s\\\"]*))|(-a (\"[^\\\"]*\"|[^\\s\\\"]*))|" +
            "(-c (\"[^\\\"]*\"|[^\\s\\\"]*)))){3}\\s*"),
    LOGIN_USER("^\\s*user\\s+login(?=.*-u (?<username>\"[^\"]*\"|[^\\s\"]*))" +
            "(?=.*-p (?<password>\"[^\"]*\"|[^\\s\"]*))(?=.*(?<status>--stay-logged-in))?(\\s+((-u\\s+(\"[^\"]*\"|[^\\s\"]*))" +
            "|(-p (\"[^\"]*\"|[^\\s\"]*))|--stay-logged-in)){2,3}\\s*$"),
