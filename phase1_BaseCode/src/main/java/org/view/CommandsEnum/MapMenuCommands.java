...
public enum MapMenuCommands {
    ...
    CREATE_NEW_MAP("\\s*create\\s+new\\s+map(?=.*-n (?<mapName>\"[^\\\"]*\"|[^\\s\\\"]*))" +
            "(?=.*-s (?<size>\\d+))(\\s+((-n (\"[^\\\"]*\"|[^\\s\\\"]*))|(-s (\\d+)))){2}\\s*"),
    CUSTOM_EXITING_MAP("^\\s*custom\\s+map\\s+-n\\s+(?<mapName>\"[^\"]*\"|[^\\s\"]*)"),
    SET_CELL_TEXTURE("^\\s*set\\s+texture(?=.*-x\\s+(?<x>\\d+))(?=.*-y\\s+(?<y>\\d+))(?=.*-t\\s+" +
            "(?<type>\"[^\"]*\"|[^\\s\"]*))(\\s+((-x\\s+(\\d+))|(-y\\s+(\\d+))|(-t\\s+(\"[^\"]*\"|[^\\s\"]*)))){3}\\s*$"),
    SET_AREA_TEXTURE("\\s*set\\s+texture(?=.*-x1 (?<x1>\\d+))(?=.*-y1 (?<y1>\\d+))" +
            "(?=.*-x2 (?<x2>\\d+))(?=.*-y2 (?<y2>\\d+))(?=.*-t (?<type>\"[^\"]*\"|[^\\s\"]*))" +
            "(\\s+((-x1 (\\d+))|(-y1 (\\d+))|(-y1 (\\d+))|(-x2 (\\d+))|(-y2 (\\d+))|(-t (\"[^\"]*\"|[^\\s\"]*)))){5}\\s*"),
